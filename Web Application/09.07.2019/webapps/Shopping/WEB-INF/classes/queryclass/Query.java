...
public class Query{
  ...
  public void addToCart(int uid,int itid)throws SQLException{
    ...
    PreparedStatement pstmt2 = null;
    try{
      String query1 = "SELECT price FROM items WHERE id = ?";
      pstmt1 = con.prepareStatement(query1);
      pstmt1.setInt(1,itid);
      ResultSet rs = pstmt1.executeQuery();
      rs.next();
      String query2 = "INSERT INTO cart (uid,item_id,cquantity,cprice) VALUES (?,?,?,?)";
      pstmt2 = con.prepareStatement(query2);
      pstmt2.setInt(1,uid);
      pstmt2.setInt(2,itid);
      pstmt2.setInt(3,1);
  ...
  public void updateCart(int uid,String iid,String cquan,String cprice)throws SQLException{
    PreparedStatement pstmt = null;
    int iiid = Integer.parseInt(iid);
    int icquan = Integer.parseInt(cquan);
    int icprice = Integer.parseInt(cprice);
    try{
      String query = "UPDATE cart SET cquantity = ?,cprice = ? WHERE uid = ? AND item_id = ?";
      pstmt = con.prepareStatement(query);
  ...
  public List<Cart> viewCart(int uid)throws SQLException{
    ...
    PreparedStatement pstmt = null;
    try{
      String query = "SELECT items.*,cart.cquantity,cart.cprice FROM cart JOIN items ON cart.item_id = items.id WHERE uid = ?";
      pstmt = con.prepareStatement(query);
      pstmt.setInt(1,uid);
  ...
  //  UUID uuid = UUID.randomUUID();
    ...
    try{
      ...
      String query1 = "SELECT item_id,cquantity,cprice FROM cart WHERE uid = ?";
      pstmt1 = con.prepareStatement(query1);
      pstmt1.setInt(1,uid);
      ResultSet rs = pstmt1.executeQuery();
      while(rs.next()){
        ...
        String query3 = "INSERT INTO purchases (pid,user_id,item_id,pquantity,pprice) VALUES (CAST(? AS UUID),?,?,?,?)";
        pstmt3 = con.prepareStatement(query3);
        pstmt3.setObject(1,pid);
        pstmt3.setInt(2,uid);
        pstmt3.setInt(3,rs.getInt("item_id"));
        pstmt3.setInt(4,rs.getInt("cquantity"));
        pstmt3.setInt(5,rs.getInt("cprice"));
    ...
    return Arrays.asList(email,pid);
  }
  ...
  public List<Order> viewCheckout(Object pid)throws SQLException{
    ...
    PreparedStatement pstmt = null;
    try{
      String query1 = "SELECT items.id,items.itemname,purchases.pquantity,purchases.pprice FROM purchases JOIN items ON purchases.item_id = items.id WHERE pid = CAST(? AS UUID)";
